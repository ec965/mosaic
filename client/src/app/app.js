...
export const PixelApp = ({pixelMap, pixelSize, borderRadius, grid, backgroundColor}) => {
  ...
  const render = pixelMap.map((inner,i) => {
    return(
      <Row key={i} style={{'backgroundColor': `${backgroundColor}`}}>
        {inner.map((p,j) => <PixelDiv grid={grid} key={j} pixel={p} pixelSize={pixelSize} borderRadius={borderRadius} backgroundColor={backgroundColor}/>)}
      </Row>
    );
  });
