...
const UpdatePasswordForm = () => {
  ...
  return(
    <>
      <Formik
        ...
        validate={(values) => {
          ...
          if(values.currPassword === values.newPassword){
            errors.newPassword = "Please use a different password from your old password.";
          }
          if (values.newPassword === state.username){
            errors.newPassword = "Please do not use your username as your password.";
          }
          if (!values.newPassword) {
            errors.newPassword = "Required";
          }
          if (values.newPassword.length < 6){
            errors.newPassword = "Please create a password of at least 6 characters.";
          }
      ...
      {authError && <FormError className="form-bottom-error">{authError}</FormError>}
    </>
  );
};
